...
def main():
    ...
    while(True):
        ...
        status, data, session = check_connection2(session)
        if not data["probe"]:
            data = {}
            data["error"] = "No hay equipos conectados"
            res = response2(command, Status.ERROR.name, data, source)
            returnResult(res)
            continue

        if command == CONNECTION:
            res = response2(command, status, data, source)
            returnResult(res)
        else:
            json_data = None
            status = data = None
            if command == CONNECT_TARGET:
                status, data, session = connect_target(session)

            elif command == DISCONNECT_TARGET:
                status, data, session = disconnect_target(session)
                ...
                status, data = set_elf_file(session, path)

            elif command == PROGRAM:
                path = parsed_stream_data['path']
                status, data = program_firmware(session, path)

            elif command == RESET:#TODO: reset solo o reset and halt?
                status, data = reset(session)

            elif command == HALT:
                status, data = halt(session)

            elif command == RESUME:
                status, data = resume(session)
            
            elif command == MONITOR:
                variables = parsed_stream_data['variables']
                status, data = read_variables(session, variables)

            elif command == 'PRENDER':
                variable = parsed_stream_data['variable']
                status, data = light_led(session, variable)
            
            returnResult(response2(command, status, data, source))

def response2(command, status, data, source):
    response = {}
...
def read_variables(session, variables):
    ...
    return Status.OK.name, variables_res
